...
df['delta'] = [p*0.01*(random.random()-0.5) for p in df['population']]
...
cs = plotly.colors.make_colorscale(['rgb(0,0,255)', 'rgb(255,0,0)'], [0.0, 1.0])
cmin = 0
cmax = 6000000

#clamp and interpolate
def clamp_interp(x, low, high):
  if x < low:
    x = low
  if x > high:
    x = high
...
def get_arrow_colors(cs, points, deltas, delta_scale):
  #angles = [math.atan(d*delta_scale)*2/math.pi for d in deltas]

  #values = [clamp_interp(d*1000 + (cmin+cmax)/2, cmin, cmax) for p, d in zip(points, deltas)]
  values = [0 if d<0 else 1 for d in deltas]

  colors = plotly.colors.sample_colorscale(cs, values)

  alphas = [math.fabs(d)/p * delta_scale for d,p in zip(deltas, points)]

  return [s.replace('rgb', 'rgba')[:-1] + ', ' + str(a) + ')' for s,a in zip(colors,alphas)]
...
standardDataDict['Air traffic geo'] = dataLayer('arcs', get_geo_arc_arrows(airport_great_circles, render='geo'))
...
# standardDataDict['Airports_Geo'] = dataLayer('scatter',
#                         [go.Scattergeo(lon = airports['Longitude'],
#                            lat = airports['Latitude'],
#                            text = airports['Name'],
#                            mode = 'markers',
#                            marker_size = 10,
...
standardDataDict['Population Change'] = dataLayer('scatter',
                        [go.Scattergeo(lon = df['lng'],
                              lat = df['lat'],
                              # #text = df['text'],
                              # #text = ' \u2B08',
                              #text = ['\u2B0A', '\u2B08','\u279E'],
                              #text = '\u279E',
                              text = ['\u2B0A' if d<0 else '\u2B08' for d in df['delta']],
                              textposition = 'middle center',
                              textfont = {'family':'Arial',
                                          'size':40,
                                          #'color': 'rgba(255, 0, 0, 0.5)'},
                                          'color': get_arrow_colors(cs, df['population'], df['delta'], 100)},
                              mode = 'text+markers',
                              ...
                              marker_colorbar = {'title': 'Population'},
                              marker_cmin = cmin,
                              marker_cmax = cmax,
                              line_color = 'red',
                              line_width = 5)]
...
layoutDict['Orthographic'] = go.Layout(
